...
function PathwayService:AssignInitialPathway(player, pathway)
	...
	self:Apply(player, pathway, initialSequence)

	return true
end

--// Assign Pathway Private API
function PathwayService:Apply(player, pathway, sequence)
	...
	self:ConnectAttributeSignals(player, self.PlayerData[player].Profile)
end

----

function PathwayService:Clear(player)
...
function PathwayService:ConnectAttributeSignals(player, profile)
	local conns = {}
	...
	table.insert(conns, player:GetAttributeChangedSignal('Pathway'):Connect(function()
		...
		self:Apply(player, newPathway, sequence)
	end))
	...
	table.insert(conns, player:GetAttributeChangedSignal('Sequence'):Connect(function()
		...
		self:Apply(player, pathway, newSequence)
	end))
...
function PathwayService:SetupPlayer(player, profile)
	...
	if pathway ~= '' then
		self:Apply(player, pathway, sequence)
	end
	...
	self:ConnectAttributeSignals(player, profile)
end
